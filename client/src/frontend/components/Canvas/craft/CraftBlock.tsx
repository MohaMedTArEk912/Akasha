...
export const CraftBlock: React.FC<React.PropsWithChildren<CraftBlockProps>> = (props) => {
    ...
    const containerEditorClasses = isContainer && enabled
        ? "border border-dashed border-slate-300/70 rounded-lg bg-slate-50/30 min-h-[60px] p-2"
        : "";
            ...
            {isContainer && (
                <div className="craft-children min-h-[40px]">
                    ...
                    {(!children || (Array.isArray(children) && (children as any[]).length === 0)) && enabled && (
                        <div className="flex flex-col items-center justify-center py-6 text-slate-400 select-none pointer-events-none">
                            <svg className="w-5 h-5 mb-1.5 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 4v16m8-8H4" />
                            </svg>
                            <span className="text-[10px] font-medium tracking-wide uppercase opacity-60">Drop blocks here</span>
                        </div>
                    )}
                </div>
            )}
