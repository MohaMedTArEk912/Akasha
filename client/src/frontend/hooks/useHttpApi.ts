...
export const httpApi = {
  ...
  deleteProjectById: async (id: string, deleteFromDisk?: boolean) => {
    const res = await client.delete(`/project/${id}`, {
  ...
  updateBlockProperty: async (
    _blockId: string,
    _property: string,
    _value: unknown,
  ) => {},
  updateBlockStyle: async (
    _blockId: string,
    _style: string,
    _value: string,
  ) => {},
  ...
  getLogicFlows: async () => {
    ...
    const res = await client.get("/logic-flows", {
  ...
  createLogicFlow: async (name: string, context: "frontend" | "backend") => {
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.post("/logic-flows", {
      projectId: activeProjectId,
      name,
      context,
    });
  ...
  getModels: async () => {
    ...
    const res = await client.get("/data-models", {
      params: { projectId: activeProjectId },
    });
    return res.data;
  },
  addDataModel: async (name: string) => {
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.post("/data-models", {
      projectId: activeProjectId,
      name,
    });
    return res.data;
  ...
  addApi: async (_method: string, _path: string, _name: string) => {
    throw new Error("Not implemented");
  },
  updateEndpoint: async (_id: string, _updates: any) => {
    throw new Error("Not implemented");
  },
  ...
  generateFrontend: async () => {
    ...
    const res = await client.post("/codegen/export", {
      projectId: activeProjectId,
    });
    return res.data;
  },
  ...
  createComponent: async (
    ...
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.post("/components", {
      projectId: activeProjectId,
      name,
      description,
    });
  ...
  getComponent: async (_id: string) => {
    throw new Error("Not implemented");
  },
  instantiateComponent: async (_componentId: string, _parentId?: string) => {
    throw new Error("Not implemented");
  },
  ...
  gitHistory: async (limit?: number) => {
    ...
    const res = await client.get(`/git/${activeProjectId}/history`, {
      params: { limit },
    });
    return res.data;
  },
  gitRestore: async (_commitId: string) => {
    throw new Error("Not implemented");
  },
  gitDiff: async (commitId: string) => {
    if (!activeProjectId) return "";
    const res = await client.get(`/git/${activeProjectId}/diff`, {
      params: { commitId },
    });
    return res.data;
  },
  ...
  gitCommit: async (message: string) => {
    ...
    const res = await client.post(`/git/${activeProjectId}/commit`, {
      message,
    });
    return res.data;
  },
  ...
  listDiagrams: async () => {
    if (!activeProjectId) return [];
    const res = await client.get("/diagrams", {
      params: { projectId: activeProjectId },
    });
    return res.data;
  },
  createDiagram: async (name: string) => {
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.post("/diagrams", {
      projectId: activeProjectId,
      name,
    });
    return res.data;
  ...
  readDiagram: async (name: string) => {
    if (!activeProjectId) return "";
    const res = await client.get(`/diagrams/${name}`, {
      params: { projectId: activeProjectId },
    });
    return res.data;
  },
  ...
  deleteDiagram: async (name: string) => {
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.delete(`/diagrams/${name}`, {
      params: { projectId: activeProjectId },
    });
    return res.data.success;
  },
  analyzeDiagram: async (_name: string) => ({
    graph: { nodes: [], edges: [] },
    issues: [],
    stats: {
      total_nodes: 0,
  ...
  analyzeDiagramRaw: async (_xml: string) => ({
    graph: { nodes: [], edges: [] },
    issues: [],
    stats: {
      total_nodes: 0,
  ...
  createUseCase: async (data: any) => {
    if (!activeProjectId) throw new Error("No active project");
    const res = await client.post("/usecases", {
      ...data,
      projectId: activeProjectId,
    });
    return res.data;
